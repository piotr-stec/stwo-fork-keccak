...
pub fn prove_poseidon(
    ...
    println!("Lookup elements: {:?}", lookup_elements);
    println!("Claimed sum from interaction trace: {}", claimed_sum);
    println!("is first id {:?}", is_first_id.id);
    println!("is active id {:?}", is_active_id.id);
    println!("Number of active rows (rounded): {}", n_active_rounded);
    // Prove constraints.
