...
#[instrument(skip_all)]
pub fn prove_with_composition_polynomial<B: BackendForChannel<MC>, MC: MerkleChannel>(
    components: &[&dyn ComponentProver<B>],
    channel: &mut MC::C,
    mut commitment_scheme: CommitmentSchemeProver<'_, B, MC>,
    ...
    let mut tree_builder = commitment_scheme.tree_builder();
    tree_builder.extend_polys(composition_poly.clone().into_coordinate_polys());
    tree_builder.commit(channel);
    span.exit();
