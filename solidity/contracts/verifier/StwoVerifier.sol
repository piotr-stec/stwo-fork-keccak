...
contract STWOVerifier {
    ...
    function _computeSamplePoints(
        ...
        internal 
        returns (FrameworkComponentLib.SamplePoints memory) 
    {
        // Get evaluator
        IFrameworkEval evaluator = IFrameworkEval(params.evaluator);
        uint32 logSize = evaluator.logSize();
        ...
        // Compute mask points
        FrameworkComponentLib.SamplePoints memory samplePoints = _componentState.maskPoints(oodsPoint);

        // Add composition polynomial tree (SECURE_EXTENSION_DEGREE = 4 columns)
        CirclePoint.Point[][][] memory newPoints = new CirclePoint.Point[][][](nTrees + 1);
    ...
    function _zipSamplePointsWithValues(
        ...
        internal 
        pure 
        returns (FriVerifier.PointSample[][][] memory samples) 
    {
        require(samplePoints.points.length == sampledValues.length, "Tree count mismatch");
        ...
        for (uint256 treeIdx = 0; treeIdx < samplePoints.points.length; treeIdx++) {
            require(
                samplePoints.points[treeIdx].length == sampledValues[treeIdx].length,
                "Column count mismatch"
            );
            
            samples[treeIdx] = new FriVerifier.PointSample[][](samplePoints.points[treeIdx].length);
            
            // For each column
            for (uint256 colIdx = 0; colIdx < samplePoints.points[treeIdx].length; colIdx++) {
                CirclePoint.Point[] memory columnPoints = samplePoints.points[treeIdx][colIdx];
                QM31Field.QM31[] memory columnValues = sampledValues[treeIdx][colIdx];
                
                require(columnPoints.length == columnValues.length, "Sample count mismatch");
                
                samples[treeIdx][colIdx] = new FriVerifier.PointSample[](columnPoints.length);
                
                // Zip points with values: zip(sampled_points, sampled_values).map(|(point, value)| PointSample { point, value })
                for (uint256 sampleIdx = 0; sampleIdx < columnPoints.length; sampleIdx++) {
                    samples[treeIdx][colIdx][sampleIdx] = FriVerifier.PointSample({
        ...
        returns (bool) 
    {
        ...
        require(QM31Field.eq(finalResult, compositionPodsEval), "OODS values do not match");
        return true; 
    }
        ...
        returns (bool) 
    {
        ...
        // TODO: Verify decommitments and queried values against FRI verifier state
        uint32[][][] memory nColumnsPerLogSizeData = getNColumnsPerLogSize(_commitmentScheme);
        uint32[][] memory commitmentColumnLogSizes = _commitmentScheme.columnLogSizes();
        FriVerifier.QueryPositionsByLogSize memory queryPositions = _friVerifier.sampleQueryPositions(_channel);
       ...
       QM31Field.QM31[][] memory friAnswersResult = FriVerifier.friAnswers(
        ...
        bool decommitSuccess = FriVerifier.decommit(_friVerifier, friAnswersResult);
